...
const fillForm = (data: {name: string; email: string; password: string; repeatpassword: string}): void => {
  userEvent.type(screen.getByPlaceholderText(EN.AUTH.NAME), data.name);
  userEvent.type(screen.getByPlaceholderText(EN.AUTH.EMAIL), data.email);
  userEvent.type(screen.getByPlaceholderText(EN.AUTH.PASSWORD), data.password);
  userEvent.type(screen.getByPlaceholderText(EN.AUTH.REPEATPASSWORD), data.repeatpassword);
...
describe("Register", () => {
  ...
  test("displays error message when user is register fail", async () => {
    ...
    fillForm(defaultData);
    await waitFor(() => {
      expect(screen.getByText(EN.AUTH.REGISTERERROR)).toBeInTheDocument();
    });
  });
  ...
  test("displays error message when name is invalid", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(EN.AUTH.NAMEERROR)).toBeInTheDocument();
    });
  });
