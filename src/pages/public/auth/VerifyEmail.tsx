...
const VerifyEmail = (): JSX.Element => {
  ...
  const handleReSendCode = async (): Promise<void> => {
    ...
    if (!isUser(response?.data)) {
      setAlert({
        type: "error",
        text: EN.AUTH.SENDCODEERROR,
      });
    } else {
      console.log({code: response.data.code});
      setAlert({
        type: "success",
        text: `${EN.AUTH.CODESENDED} ${values.email}`,
      });
    }
    setLoading(false);
  };
  ...
  return (
    <AuthLayout title={EN.PAGES.VERIFYEMAIL.TITLE}>
      <AuthForm<VerifyEmailValues>
        loading={loading}
        alert={alert}
        submitText={EN.PAGES.VERIFYEMAIL.BUTTON}
        onSubmit={async e => {
        ...
        extraButton={
          <Button
            ...
            type="button"
            text={EN.PAGES.VERIFYEMAIL.BUTTONRESEND}
            onClick={async () => {
