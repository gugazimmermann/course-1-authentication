...
const Blog = (): JSX.Element => {
  ...
  const ferchMoviesList = useCallback(async (gId?: string): Promise<void> => {
    const moviesURL = `${String(process.env.REACT_APP_TMDB_ENDPOINT)}movie/now_playing?page=1`;
    const genresMoviesURL = `${String(process.env.REACT_APP_TMDB_ENDPOINT)}discover/movie?include_adult=true&include_video=true&page=1&sort_by=popularity.desc&with_genres=${gId}`;
    const genresURL = `${String(process.env.REACT_APP_TMDB_ENDPOINT)}genre/movie/list`;
  ...
  const renderContent = (): JSX.Element => {
    ...
    return <Alert text={EN.PAGES.BLOG.NORESULT} type="info" />;
  };

  return (
    <section className="w-full">
      {postId === undefined && <Title title={`${EN.PAGES.BLOG.TITLE}${genre?.length > 0 ? ` - ${genre}` : ""}`} />}
      {renderContent()}
    </section>
  );
};
