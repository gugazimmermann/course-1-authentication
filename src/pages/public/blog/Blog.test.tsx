...
describe("Blog", () => {
  ...
  it("fetches movies and genres with genre id and displays", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(`${EN.PAGES.BLOG.TITLE} - ${genresList[1].name}`)).toBeInTheDocument();
      expect(screen.getByText(movie.title)).toBeInTheDocument();
    });
  });
  ...
  it("fetches with genre id and have no movie", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(`${EN.PAGES.BLOG.TITLE} - ${genresList[0].name}`)).toBeInTheDocument();
      expect(screen.getByText(EN.PAGES.BLOG.NORESULT)).toBeInTheDocument();
    });
  });
  ...
  it("handles exception in fetch call", async () => {
    ...
    setupComponent(ROUTES.BLOG);
    await waitFor(() => {
      expect(screen.getByText(EN.PAGES.BLOG.ERROR)).toBeInTheDocument();
    });
  });
