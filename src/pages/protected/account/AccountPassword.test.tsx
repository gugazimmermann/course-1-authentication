...
const setupAccountPasswordTest = async ({
  ...
  const inputPassword = screen.getByPlaceholderText(EN.AUTH.CURRENTPASSWORD);
  const inputNewPassword = screen.getByPlaceholderText(EN.AUTH.NEWPASSWORD);
  const inputRepeatPassword = screen.getByPlaceholderText(EN.AUTH.REPEATNEWPASSWORD);
  ...
  if (formError === "password") {
    expect(screen.getByText(EN.AUTH.PASSWORDERROR)).toBeInTheDocument();
  } else if (submitError) {
    await waitFor(() => {
      expect(screen.getByText(EN.AUTH.PASSWORDCHANGEERROR)).toBeInTheDocument();
    });
  } else {
    await waitFor(() => {
      expect(screen.getByText(EN.AUTH.PASSWORDCHANGESUCCESS)).toBeInTheDocument();
    });
  }
};
