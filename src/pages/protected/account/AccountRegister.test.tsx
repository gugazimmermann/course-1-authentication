...
const setupAccountRegisterTest = async ({
  ...
  const inputName = screen.getByPlaceholderText(EN.AUTH.NAME);
  const inputEmail = screen.getByPlaceholderText(EN.AUTH.EMAIL);
  ...
  if (formError === "name") {
    ...
    await waitFor(() => {
      expect(screen.getByText(EN.AUTH.CHANGEUSERDATAERROR)).toBeInTheDocument();
    });
  } else if (!isVerified) {
    await waitFor(() => {
      expect(screen.getByText(EN.AUTH.CHANGEUSERDATAEMAIL)).toBeInTheDocument();
    });
  } else {
    await waitFor(() => {
      expect(screen.getByText(EN.AUTH.CHANGEUSERDATASUCCESS)).toBeInTheDocument();
    });
  }
};
